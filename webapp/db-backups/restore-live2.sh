...
# Dump lives in PROJECT ROOT db-backups/. Everything read; no env/creds echoed.
set +H
WA=$(ls -d /Users/*/workstations/newsportal/webapp | sed -n '1p')
ROOT=$(dirname "$WA")
LOG="$WA/db-backups/restore-live2.log"
: > "$LOG"
LIVE=$(sed -nE '/^#[[:space:]]*DATABASE_URL=/{s/^#[[:space:]]*DATABASE_URL=//;p}' "$WA/.env" | sed -n '1p')
...
{
  echo "===== 0 pre: live still empty (read-only) ====="
  PGCONNECT_TIMEOUT=60 psql "$LIVE" -X -tAc "SELECT 'users='||count(*) FROM users UNION ALL SELECT 'articles='||count(*) FROM articles UNION ALL SELECT 'live_streams='||count(*) FROM live_streams;" 2>&1 | mask

  echo "===== 1 parent tables ====="
  ...
  PGCONNECT_TIMEOUT=60 psql "$LIVE" -X -tAc "
    SELECT 'users='||count(*) FROM users;" >/dev/null
  PGCONNECT_TIMEOUT=60 psql "$LIVE" -X -tAc "
...
echo "restore2_log_bytes=$(wc -c <"$LOG")"